...
> 将 FlClash 和 Mihomo（Clash Meta）代理配置合并为一个文件，实现精确的域名路由
> 和分流隧道——支持多订阅合并，隐私安全。

## 使用场景

- **一个订阅用于日常，另一个订阅用于特定网站。** 保持主订阅处理常规流量，同时将
  单个网站通过另一个提供商的节点路由。
- **首选主节点，精确分流。** 将 NotebookLM、Gemini、ChatGPT 或其他域名通过已验证
  的节点路由，不影响其余流量。
- **将多个订阅合并为一个 FlClash 兼容的本地配置。** 将两个或更多订阅配置合并为
  FlClash 可直接加载的单个文件。
- **避免宽泛的 Google 域名规则意外劫持无关服务。** 工具默认阻止 `google.com` 等
  宽泛规则，仅对已确认的目标域名进行分流。
...
| 合并前 | 合并后 |
|---|---|
| FlClash 同一时间只能激活一个配置文件。 | 一个合并后的配置，包含主节点组、辅助节点组和目标分流组。 |
| 切换订阅意味着丢失一组节点。 | 两个订阅的所有节点共存于单个配置中，支持精确的域名级路由。 |
...
- **学生和研究人员** — 需要通过可靠代理节点访问 NotebookLM 或 Gemini 等地区敏感
  服务的用户。
- **开发者** — 管理多个代理订阅，用于不同项目或环境的用户。
- **拥有地区敏感网络服务的用户** — 希望精确控制哪些域名通过哪些节点路由的用户。
...
本项目涵盖 FlClash 分流隧道、Mihomo 规则与路由、Clash Meta 配置合并、多订阅
代理管理、基于域名的代理路由，以及通过策略组实现的 NotebookLM 代理路由。
